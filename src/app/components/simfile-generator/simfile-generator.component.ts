...
// import { Rank, Tensor, loadGraphModel, zeros } from '@tensorflow/tfjs'
    ...
    if (fileList && fileList.length == 1) {
      ...
      var classPromise = new Promise<void>((resolve) => {
        this.readAudio(fileList, (buffer) => {
          ...
          // loadGraphModel(modelUrl, { fromTFHub: true }).then((model)=>{
          //   const waveform = zeros([16000 * 3]);
            
          //   const [scores, embeddings, spectrogram] = model.predict(waveform) as Tensor<Rank>[];
